...
You can click this link to [join the iRacing Stats Discord server](https://irstatsbot.fuzzwah.com/discord) where the bot will respond to commands in the [b]#bot-playground[/b] channel. Feel free to check out your own stats for the current season using the `!driver Your Name` command. Below is an example of the type of information that the bot will respond with:
